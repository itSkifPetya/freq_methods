...
В данном задании используется унитарное преобразование Фурье к угловой частоте $\omega$:
\[
\hat{g}(\omega) = \frac{1}{\sqrt{2\pi}} \int_{-\infty}^{\infty} g(t)e^{-i\omega t}dt
\]

Была выбрана функция кардинальный синус:
\[
f(t) = 3\,\mathrm{sinc}(t)
\]

Исследуемая функция --- сдвинутый кардинальный синус:
\[
g(t) = f(t + c) = 3\,\mathrm{sinc}(t + c)
\]

Исследование проводилось для трёх значений сдвига: $c \in \{0, 1, 2\}$.

\subsection{Аналитические выражения и свойства преобразования}

Согласно свойству сдвига преобразования Фурье, если функция сдвинута на величину $c$ во временной области, то её Фурье-образ умножается на фазовый множитель:
\[
\hat{g}(\omega) = e^{i\omega c} \cdot \hat{f}(\omega)
\]

Образ кардинального синуса в унитарной нормировке является конечной функцией в частотной области. Фазовый множитель $e^{i\omega c} = \cos(\omega c) + i\sin(\omega c)$ изменяет вещественную и мнимую части образа, но не влияет на его модуль:
\[
|\hat{g}(\omega)| = |e^{i\omega c}| \cdot |\hat{f}(\omega)| = |\hat{f}(\omega)|
\]

\subsection{Методика вычислений}

Унитарное преобразование Фурье вычислялось численно методом трапеций:
...
Параметры: временная сетка $t \in [-10, 10]$ с 3000 точками, шаг по частоте $\Delta\omega = 0.05$, $\omega_{\max} = 8$ рад/с.
...
\begin{figure}[H] 
    \includegraphics[width=1\textwidth]{graphs/figure_3.pdf}
    \caption{Оригиналы функции $g(t) = 3\,\mathrm{sinc}(t + c)$}
    \label{fig:complex_originals}
\end{figure}

На рис.~\ref{fig:complex_fourier0},~\ref{fig:complex_fourier1},~\ref{fig:complex_fourier2} показаны компоненты Фурье-образа для каждого значения $c$: вещественная часть, мнимая часть и модуль.

\begin{figure}[H] 
    \includegraphics[width=1\textwidth]{graphs/figure_4.pdf}
    \caption{Фурье-образ при $c = 0$}
    \label{fig:complex_fourier0}
\end{figure}

\begin{figure}[H] 
    \includegraphics[width=1\textwidth]{graphs/figure_5.pdf}
    \caption{Фурье-образ при $c = 1$}
    \label{fig:complex_fourier1}
\end{figure}

\begin{figure}[H] 
    \includegraphics[width=1\textwidth]{graphs/figure_6.pdf}
...
Модуль образа $|\hat{g}(\omega)|$ во всех трёх случаях практически идентичен. Это подтверждает, что фазовый множитель $e^{i\omega c}$ не влияет на амплитуду спектра, только на его фазу. Модуль сосредоточен в области низких частот и затухает при увеличении $\omega$.

При $c = 0$ функция чётная, поэтому её образ вещественный: $\mathrm{Im}(\hat{g}(\omega)) \approx 0$. При $c > 0$ фазовый множитель вводит комплексность:
\[
\mathrm{Re}(\hat{g}(\omega)) = |\hat{f}(\omega)| \cos(\omega c), \quad \mathrm{Im}(\hat{g}(\omega)) = |\hat{f}(\omega)| \sin(\omega c)
\]

Величина сдвига $c$ определяет скорость осцилляции фазы. С увеличением $c$ от 1 к 2 вещественная и мнимая части чередуются интенсивнее.

Численные результаты подтверждают теоретическое свойство сдвига преобразования Фурье: сдвиг сигнала во времени приводит к сдвигу фазы образа без изменения его модуля. Это свойство играет важную роль при выполнении задач, где фазовая информация более значима, чем амплитудная.
...
\begin{figure}[H] 
    \includegraphics[width=1\textwidth]{graphs/figure_1.pdf}
...
\begin{figure}[H] 
    \includegraphics[width=1\textwidth]{graphs/figure_2.pdf}
...
\begin{itemize}
    \item При пороге 0.002 (перцентиль 97\%), было обнаружено 32 пика, в котором помимо нужных нам 4 нот имеются "выбросы" F\#3, G\#2, A4. Данные пики могли возникнуть из-за особенностей звукоизвлечения при игре гитаре. В момент удара по струнам помимо звука самих зажатых струн имеется звук самого удара, который может так же проявляться пиком на графике Фурье-образа, смешиваясь с негромкими оттенками аккорда.
...
plt.title(r"Оригиналы $g(t) = \mathrm{sinc}(t + c)$", fontsize=14)
plt.xlabel(r"$t$")
plt.ylabel(r"$g(t)$")
...
for c in c_values:
    ...
    plt.title(rf"Фурье-образ $\hat{{g}}(\omega)$ при $c = {c}$", fontsize=14)
